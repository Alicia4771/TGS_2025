...
public class CourseIntroduction : MonoBehaviour
{
    ...
    private double uchiggawa_time = 3;      // 何秒後に内側に移動するか
    private float x_force = 0.2f;           // 内側に移動する時の速さ
    private float x_limit = 1;              // どこまでx座標を動かすか

    private double sageru_time = 4;      // 何秒後にカメラを少し下げるか
    private float y_force = 0.02f;      // カメラを下げる速さ
    private float y_limit = 3;          // どこまでy座標を下げるか

    private double stop_time = 10;      // 何秒後に進むのをやめるか
    private double senter_time = 11;    // 何秒後に正面に振り返るか
